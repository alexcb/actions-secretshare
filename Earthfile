...
test-run:
    ...
    RUN cat output | grep '^::add-path::' | sed 's/::add-path:://g' > secretshare-path
    RUN test "$(cat secretshare-path)" = "/root/.secretshare/bin"
    # [a-zA-Z0-9]* attempt to match a commit hash
    RUN export PATH="$(cat secretshare-path):$PATH" && secretshare --version | tee version.output
    RUN grep '^secretshare version: v.*' version.output
